...
.cbox { position: relative; background: rgba(15,23,42,.8); border: 1px solid rgba(255,255,255,.12); border-radius: 28px; padding: 26px 16px 20px; text-align: center; font-family: 'Inter', system-ui, sans-serif; box-shadow: 0 24px 50px rgba(0,0,0,.4); }
...
.cbox .crown { position: absolute; top: -14px; left: 50%; transform: translateX(-50%); font-size: 26px; opacity: 0; }
#app.run .cbox.hl .crown { opacity: 1; animation: ctaPop .45s cubic-bezier(.175,.885,.32,1.5) 4.2s both; }
#app.run .cbox.hl { animation: winnerPop .5s cubic-bezier(.175,.885,.32,1.4) 4.2s both; }
@keyframes winnerPop { 0%,100% { transform: scale(1); box-shadow: 0 24px 60px rgba(4,120,87,.5), 0 0 50px rgba(16,185,129,.35); } 40% { transform: scale(1.08); box-shadow: 0 24px 80px rgba(4,120,87,.8), 0 0 90px rgba(255,210,63,.6); } }
.win {
  position: absolute; top: 1080px; left: 50%; transform: translateX(-50%) scale(.4);
  display: flex; align-items: center; gap: 8px;
  background: linear-gradient(135deg, #FFD23F, #FFB703); color: #0F172A;
  padding: 14px 24px; border-radius: 999px;
  font-family: 'Inter', system-ui, sans-serif; font-size: 17px; font-weight: 900; letter-spacing: .3px;
  box-shadow: 0 16px 40px rgba(0,0,0,.5), 0 0 40px rgba(255,210,63,.4);
  opacity: 0;
}
#app.run .win { opacity: 1; animation: ctaPop .45s cubic-bezier(.175,.885,.32,1.5) 4.2s both; }
...
function renderScene(s) {
  ...
  const bandZone = isComparar
    ...
    <div class="compare">
      ...
      <div class="cbox hl"><div class="l">${s.label2}</div><div class="v" id="rv2">0.00</div><div class="crown">&#127941;</div></div>
    </div>
    <div class="win">&#128293; ${s.winner} gana por +S/ ${s.diff} al mes</div>`
    : `
